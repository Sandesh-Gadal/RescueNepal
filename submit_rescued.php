...
$gender=$isSafe?($_POST['gender']??'Unknown'):($_POST['assisted_gender']??'Unknown');if(!in_array($gender,['Male','Female','Other','Unknown'],true))$gender='Unknown';
...
try{
 ...
 $identity=$isSafe?'known':($docName!==''?'claimed':'unknown');
$displayName=$isSafe?$name:($docName!==''?('Possible: '.$docName):('Unknown '.($gender==='Female'?'Female':($gender==='Male'?'Male':'Person'))));
 ...
 $token=random_token(20);$familyName=$isSafe?$emergencyName:($instContact!==''?$instContact:$institution);$familyPhone=$isSafe?$emergencyPhone:$instPhone;
 ...
 $stmt->execute([$displayName,$isSafe?$age:null,$gender,trim((string)($_POST['permanent_address']??'')),$location,$photo['photo']??null,$photo['thumb']??null,$familyName,$familyPhone,$notes,$status,$condition,$institution,$location,$institution,$token,$admin['id']??null]);
