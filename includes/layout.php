...
function page_header(string $title, bool $admin=false): void {
    ...
    $app=e($appDisplay);
    $brandParts=explode(' - ',$appDisplay,2);
    $brandName=e(trim($brandParts[0]));
    $brandTagline=isset($brandParts[1])?e(trim($brandParts[1])):'';
    ...
    echo '<link rel="stylesheet" href="'.e(base_url('assets/app.css?v=2.4.27')).'">';
    echo '<link rel="stylesheet" href="'.e(base_url('assets/simple-ui.css?v=2.4.27')).'">';
    ...
    echo '<span class="simple-site-title-wrap"><span class="simple-site-title">'.$brandName.'</span>'.($brandTagline!==''?'<span class="simple-site-tagline">'.$brandTagline.'</span>':'').'</span></a>';
    echo '<div class="simple-header-actions"><label class="language-control"><span>Language</span><select id="langMode" aria-label="Language"><option value="en" selected>English</option><option value="both">English + नेपाली</option><option value="np">नेपाली</option></select></label>';
    if($admin){
        echo '<a class="simple-text-link desktop-only" href="'.e(base_url('admin/logout.php')).'">Logout</a>';
    } else {
        echo '<a class="simple-text-link" href="'.e(base_url('admin/login.php')).'">Admin Login</a>';
    }
    echo '</div></div></header>';
...
function page_footer(): void {
    ...
    echo '<script src="'.e(base_url('assets/app.js?v=2.4.27')).'"></script></body></html>';
}
